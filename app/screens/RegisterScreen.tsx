...
interface RegisterReq extends RegisterRequest {
...
const RegisterScreen = () => {
    ...
    const [fontsLoaded] = useFonts({ Alegreya_400Regular_Italic });
    const [touched, setTouched] = React.useState<{ [key: string]: boolean }>({});
    const [formData, setFormData] = React.useState<RegisterReq>({
        fullname: '',
        username: '',
        email: '',
        password: '',
    ...
    const [errors, setErrors] = React.useState<Partial<RegisterReq>>({});

    if (!fontsLoaded) return null;

    const handleChange = (name: keyof RegisterReq, value: string) => {
        setFormData((prev) => ({ ...prev, [name]: value }));
        if (errors[name as keyof RegisterReq]) setErrors((prev) => ({ ...prev, [name]: undefined }));
    };

    const validateField = (name: keyof RegisterReq, value: string) => {
        const newErrors: Partial<RegisterReq> = { ...errors };

        switch (name) {
            case 'username':
                if (!value.trim()) newErrors.username = 'El usuario es obligatorio';
                else delete newErrors.username;
                break;
            case 'fullname':
                if (!value.trim()) newErrors.fullname = 'El nombre completo es obligatorio';
                else delete newErrors.fullname;
                break;
            case 'email':
                if (!value.trim()) newErrors.email = 'El correo es obligatorio';
                else if (!/\S+@\S+\.\S+/.test(value)) newErrors.email = 'El formato del correo no es válido';
            ...
            case 'password':
                ...
                else if (name === 'password' && newErrors.confirmPassword) delete newErrors.confirmPassword;
                break;
    ...
    const validateForm = (): boolean => {
        const newErrors: Partial<RegisterReq> = {};
    ...
    return (
        ...
        >
            ...
            <View style={styles.bottomSection}>
                ...
                <TouchableOpacity
                    style={[styles.registerButton, (isFormEmpty || isLoading) && styles.registerButtonDisabled]}
                    onPress={handleSubmit}
                    disabled={isFormEmpty || isLoading}
                >
                    <Text style={[styles.registerButtonText, isFormEmpty && styles.registerButtonTextDisabled]}>
                        CREAR TU CUENTA
                    </Text>
                </TouchableOpacity>
